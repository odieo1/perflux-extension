export function setup(ctx: any) {
  ...
  function buildUrl(prompt: string, neg: string, style: string, seed: number): string {
    ...
    const p = new URLSearchParams({
      ...
      seed: String(seed), nologo: "true"
    });
    if (neg.trim()) p.set("negative_prompt", neg.trim());
    return `${BASE}${encodeURIComponent(prompt.trim() + suffix)}?${p}`;
  }

  function makeCard(url: string, promptText: string, idx: number): HTMLElement {
    ...
    img.addEventListener("error", () => {
      ...
      const errEl = el("div", "", "? Failed to load", "");
      errEl.style.cssText = "font-size:11px;color:#e05a5a;padding:8px;text-align:center";
      card.appendChild(errEl);
    });
    card.addEventListener("click", () => {
      if (img.complete && img.naturalWidth) openLB(url);
    });
  ...
  genBtn.addEventListener("click", async () => {
    ...
    const urls  = Array.from({ length: count }, (_, i) =>
      buildUrl(promptVal, neg, style, seed0 + i));
    const cards = urls.map((u, i) => makeCard(u, promptVal, i));
    [...cards].reverse().forEach(c => gallery.insertBefore(c, gallery.firstChild));
